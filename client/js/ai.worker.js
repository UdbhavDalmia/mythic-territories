...
import { getValidMoves, getEffectivePower, isCaptureSuccessful } from '../../shared/utils.js';
...
function simulateAbility(gameState, piece, abilityKey, target = null) {
    ...
    // Abilities cannot be used by stuck or dazed pieces
    if ((simPiece.stuck || 0) > 0 || simPiece.isDazed) return tempState;

    if (abilityKey === 'Siphon') {
        if (simPiece.charges < (simPiece.ability?.maxCharges || 3)) {
            simPiece.charges = (simPiece.charges || 0) + 1;
        }
        if (simPiece.ability) simPiece.ability.cooldown = 1;
        return tempState;
    }
